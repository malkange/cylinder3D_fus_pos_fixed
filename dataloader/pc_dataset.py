...
class SemKITTI_sk(data.Dataset):
    def __init__(self, data_path, split='train',
        ...
        for seq in self.seqs:
            for pcd_name in sorted(os.listdir(os.path.join(self.root, seq, 'velodyne'))):
                ...
                self.img_fea_files[0].append(os.path.join(self.root, seq, 'img_fea_256', str(pcd_name[:-4]) + '.pt'))
        ...
        """ gs
        self.img_fea_ext_layer = nn.Sequential(
                nn.Linear(256, 3),
                nn.ReLU()
        )      
        """       
    ...
    def __getitem__(self, index):
        ...
        img_fea_tensor = torch.load(self.img_fea_files[0][index], map_location='cpu') # torch.Size([10000, 256]) 

        all_indices = torch.arange(img_fea_tensor.size(0)) # (10000,)
        zero_indices = torch.all(img_fea_tensor == 0, dim=1).nonzero(as_tuple=False).squeeze() # (2000,)
        non_zero_indices = torch.tensor(list(set(all_indices.tolist()) - set(zero_indices.tolist()))) # (8000,)

        non_zero_img_fea_tensor = img_fea_tensor[non_zero_indices] # (8000, 256)

        #gs
        #non_zero_img_fea_tensor = self.img_fea_ext_layer(non_zero_img_fea_tensor) # (8000, 3)

        #gs
        #img_fea_tensor = torch.zeros((img_fea_tensor.shape[0], 3))
        img_fea_tensor = torch.zeros((img_fea_tensor.shape[0], 256))

        img_fea_tensor[zero_indices] = 0
        img_fea_tensor[non_zero_indices] = non_zero_img_fea_tensor.clone().detach()
